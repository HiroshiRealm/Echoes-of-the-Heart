    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(API_URL, headers=headers, json=data) as response:
                ...
                print(f"记忆点 '{memory_point['title']}' 的触发结果: {answer}")
                return answer == "是"
    except Exception as e:
        print(f"检查记忆点触发时出错: {str(e)}")
        return False
...
def main():
    ...
    if command == "check_all_memory_points":
        try:
            # 从标准输入读取 JSON 数据
            input_data = json.loads(sys.stdin.read())
            # 运行异步函数
            results = asyncio.run(check_all_memory_points(input_data.get('current_dialogue', '')))
            # 输出结果
